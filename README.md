...
- **显示页 `/`**：从左侧图片列表打开 SVG，只浏览图片和标签，支持缩放、平移和全屏；标签不可点击、修改或拖动。
- **管理页 `/admin`**：添加 SVG、修改图片名称、删除图片，并添加、修改、拖动或删除文字标签。
...
3. 在 `/admin` 点击“添加标签”，在图片上点击或触摸需要标注的位置。
4. 输入文字，点击“保存标签”（也可以按 Ctrl/⌘+Enter）。支持多行文字。
5. 拖动标签可以调整位置；点击标签可以修改文字或删除。
6. 点击空白处拖动画布。普通滚轮滚动，Ctrl/⌘+滚轮以指针位置缩放，平板使用双指捏合缩放。

标签编辑操作仅在管理页提供。显示页没有“添加标签”按钮，点击或触摸标签不会弹出编辑框，标签也不会获得键盘焦点；从标签上开始拖动会平移整张图片，不会改变标签相对图片的位置。管理页保存的标签仍会自动同步到显示页。

完成标注后，可在管理页点击“锁定图片”。系统先保存当前输入的非空标签及待保存修改，成功后锁定；锁定期间不能添加、点击编辑、拖动或删除标签，从标签上拖动会平移图片，缩放和浏览正常。点击“解锁图片”后恢复编辑。锁定状态按图片保存到服务器，刷新、切换图片和服务重启后仍有效，图片列表中的锁图标表示已锁定。服务器也会拒绝对锁定图片的标签写入；原有图片和新上传图片默认未锁定。“锁定图片”与“固定缩放”分别控制标签编辑和缩放比例，可独立使用。
...
管理页和显示页都提供“固定缩放”按钮。开启后保持当前比例，放大、缩小、100%、适应宽度、Ctrl/⌘+滚轮缩放和双指缩放均停用；鼠标 / 单指拖动、普通滚轮浏览和双指平移仍可使用。隐藏侧边栏、进入全屏或改变窗口尺寸也不会改变比例。点击“解除固定缩放”后恢复缩放与最小比例限制；打开另一张图片时默认解除固定。
